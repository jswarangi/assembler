...
/*typedef BST* pointer;
	...
	string programName;
	int locCounter;
	bool isExpValid,nBit,iBit,xBit,controlSection,adjustRFlag;
	char charOperator;
	string operandArray[2],change[2];*/
...
*** RETURN      : bool, result									  ***
********************************************************************/
bool check::checkLiteral(string &operand, string &literalValue, int &literalLength)
{   bool result = false;
     literalValue=" ";
	 literalLength=0;
	 string value=" ";
	string tempName = operand;
     int i;
	if(tempName[0]== '=' )
	{    result = true;
		if(tempName[2]=='\'')
		{    result = true;
			tempName[1] = toupper(tempName[1]);
			switch(tempName[1])
			{
			case 'X':
			      for(i=3;tempName[i]!='\'';i++)
				     literalValue  += tempName[i];
				
				for (int k=0; k<literalValue.length(); k++) 
	           {
		         literalValue[k]=toupper(literalValue[k]);
	           }
				literalLength = literalValue.length();
				literalLength = (int) literalLength/2;
			...
			case 'C':	
                 for(i=3;tempName[i]!='\'';i++)
				     value += tempName[i];	
				literalValue = converttohexstr(value);
				literalLength = (tempName.length()-4);
				result = true;
				break;
			default:
				cout<<"  ERROR - Literal: "<<tempName<<"  must be character or hexadecimal."<<endl;
				
				result = false;
			}
		} // else if statement to check bound by ' ' 
		else{
			cout << "  ERROR - Literal: " <<tempName << " must be enclosed by ' ' " <<endl;
			result = false;
		}
	} // out most if
...
int check::charCount(string line, char value){
...
********************************************************************/
int check::converthexstrtoint(string strValue)
{
	int value;
	string strvalue = strValue;
	value = strtoul(strvalue.c_str(),NULL,16);
	return value;
}
	...
	string tempExp = " ",
	 ...
	 if( result)
	 {
		...
		else if( operands > 0 && comma > 0)
		{
			...
			result = false;
			cout << "  Error - expression: " << expression << " can't combine indexing with unary/binary operations. " << endl;
			    line++;
			   if(line == 20)
			   {   
		           cout << "  Input any key to continue:  " ;
	 ...
	 // only if two operands per expression
  if(result)
  {
	 ...
	 else if(expression[0] == '@')
	 {
		...
		{
			result = false;
			cout << "  Error- expression: " << expression << "  unary operation can't be used with value."<<endl;
		...
		else if(binaryOperator =='+')
		{
			...
			{
				result = false; 
				cout << "  Error - expression: " << expression << " indexing can't be done with immediate addressing" << endl;
...
*** RETURN      : string, strValue									  
********************************************************************/
bool check::parseInstructions(string insLine, string &label, string &operation, string &operand)
{
	...
	{
		if(!isspace(insLine[0])){

			while(!isspace(insLine[i])){
				label += insLine[i];
				i++;
			}
			...
			insLine = insLine.substr(i,insLine.length()-1);
			i = 0;

			while(!isspace(insLine[i]) && i < insLine.length()){
				operation += insLine[i];
				i++;
			}
			...
			while(insLine[i] != '$' && i < insLine.length()){
				operand += insLine[i];
				i++;
			}
		}
...
*** RETURN      : bool									  ***
********************************************************************/
bool check::parse(string insLine, string &lineNo, string &locCounter, string &label, string &strOpcode, string &operand)
{
...
/********************************************************************
*** FUNCTION  findRegisterValue                                   ***
*********************************************************************
*** DESCRIPTION : Finds the value of register    				  ***
*** INPUT ARGS  : string				                           ***
*** OUTPUT ARGS : none   										  ***
*** IN/OUT ARGS :		 										  ***
*** RETURN        : int										  ***
********************************************************************/
int check::findRegisterValue(string name)
{
	int value = -1; 
	for(int d=0;d<name.length();d++)
	{
	name[d] = toupper(name[d]);
	}
	if(name == "A")
		value = 0; 
	else if(name == "X")
		...
		//cout<< " Error : Invalid resister used : " << name <<endl ;
	return value;
}



void check::fiveDigitLC(string &LocCounter )
{

	if(LocCounter.length() ==1){
							LocCounter = "0000"+ LocCounter;
					}
					else if (LocCounter.length() ==2){
						LocCounter = "000"+ LocCounter;
					}
					else if(LocCounter.length() ==3){
						LocCounter = "00"+ LocCounter;
					}
					else if(LocCounter.length() ==4){
						LocCounter = "0"+ LocCounter;
					}
					else if(LocCounter.length()>6) {
						int x = LocCounter.length() - 5;
						LocCounter = LocCounter.substr(x,LocCounter.length()-1);
					}
